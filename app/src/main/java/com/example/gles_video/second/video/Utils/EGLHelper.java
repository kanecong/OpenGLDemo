...
public class EGLHelper {
    ...
    public void setDisplay(int key) {
        ...
        Log.d("gaorui", EGL14.eglQueryString(mEglDisplay, EGL14.EGL_VENDOR));
        Log.d("gaorui", EGL14.eglQueryString(mEglDisplay, EGL14.EGL_VERSION));
        Log.d("gaorui", EGL14.eglQueryString(mEglDisplay, EGL14.EGL_EXTENSIONS));
    }
    ...
    public EGLSurface createWindowSurface(EGLConfig config, Object surface) {
        ...
        if (eglSurface == EGL14.EGL_NO_SURFACE) {
            Log.d("gaorui", "createWindowSurface" + EGL14.eglGetError());
            return null;
        }
        return eglSurface;
    }
    ...
    public boolean makeCurrent(EGLSurface draw, EGLSurface read, EGLContext context) {
        if (!EGL14.eglMakeCurrent(mEglDisplay, draw, read, context)) {
            Log.d("gaorui", "makeCurrent" + EGL14.eglGetError());
            return false;
        }
        return true;
    }
    ...
    public boolean setPresentationTime(EGLSurface surface, long timeStamp) {
        if (!EGLExt.eglPresentationTimeANDROID(mEglDisplay, surface, timeStamp)) {
            Log.d("gaorui", "setPresentationTime" + EGL14.eglGetError());
            return false;
        }
        return true;
    }
    ...
    public boolean swapBuffers(EGLSurface surface) {
        if (!EGL14.eglSwapBuffers(mEglDisplay, surface)) {
            Log.d("gaorui", "swapBuffers" + EGL14.eglGetError());
            return false;
        }
        return true;
    }
